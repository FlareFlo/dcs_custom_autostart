...
----------------------------------------------------------------------------------------------------
-- Start sequence
push_start_command(2.0,	{message = _("CUSTOM AUTOSTART SEQUENCE IS RUNNING"), message_timeout = start_sequence_time})
--
...
push_start_command(dt,		{device = devices.MAP,					action = map_commands.PwrSw,					value = 1.0}) -- not used, but i like it :)
...
push_start_command(dt,		{message = _("- WAITING FOR INS ALIGN"),												message_timeout = 80.0})
push_start_command(80.0,	{message = _("- CHECK INS ALIGNMENT - READY"),			check_condition = F16_AD_INS_CHECK_RDY,			message_timeout = 5.0})
...
-- Enables Link16 / XMT16
push_start_command(dt,		{device = devices.MFD_RIGHT,					action = mfd_commands.OSB_6,					value = 1.0})
push_start_command(dt,		{device = devices.MFD_RIGHT,					action = mfd_commands.OSB_6,					value = 0.0})

-- Enables RWR
push_start_command(dt,		{device = devices.RWR,					action = rwr_commands.Power,					value = 1.0})

-- Enables RWR-sub-modules
...
push_start_command(dt,		{device = devices.CMDS,					action = cmds_commands.MwsSrc,					value = 1.0}) -- Does nothing, but i like it

-- Sets semi-automatic countermeasures mode
push_start_command(dt,		{device = devices.CMDS,					action = cmds_commands.Mode,					value = 0.3})

-- Enables chaff dispensing
push_start_command(dt,		{device = devices.CMDS,					action = cmds_commands.ChExp,					value = 1.0})

-- Power up targeting pod pylons
...
-- Set altitude scale to use electric / radar altitude
push_start_command(dt,		{device = devices.AAU34,					action = alt_commands.ELEC,					value = -1.0})
push_start_command(1.0,		{device = devices.AAU34,					action = alt_commands.ELEC,					value = 0.0})
